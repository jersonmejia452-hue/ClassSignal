-- Keep rejected anonymous submissions inside the abuse counter transaction.
-- The v1 RPC raised exceptions after incrementing its bucket, which rolled the
-- increment back together with the request. Returning a bounded outcome lets
-- PostgreSQL commit the attempt while the Edge Function still exposes the
-- appropriate HTTP status.

create index response_submission_buckets_window_started_at_idx
  on private.response_submission_buckets (window_started_at);
...
begin
  ...
  -- Serializes active-state, capacity, duplicate, and bucket checks per class.
...
comment on function public.submit_student_response_server_v2(
  uuid, uuid, text, text, text
) is 'Server-only atomic submission whose bounded outcomes preserve rejected attempts in the abuse counter.';
